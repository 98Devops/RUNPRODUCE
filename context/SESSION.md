...
Last updated: 2026-09-23 (UI sprint: U9 v1 standing; U6 set down at chunk 7 red, pushed) · Branch: `u9-first-screen` (from main). U6 lives on `u6-supabase-schema`
...
- **U9 v1, 2026-09-23** (`u9-first-screen`): three cards from fixture 7, hardcoded, no DB, no auth. Tracker entry has the detail. This branch: engine 344 (U6's +4 not merged), `apps/web` 23 green, build static.
- **Status (`u6-supabase-schema`):** engine 348 unit tests; golden 11 written / 11 passing / 1 held; lint clean. `apps/web` unit suite 49 red and `tsc` red on 9 enum exports, by design (chunk 7 red phase). DB suite 35 red / 37 green of 72, by design.
...
- **Chunk 7** (repositories, D25-D30): **approved 2026-09-15**, AD-90 to AD-95.
  ...
  - **RED PHASE COMMITTED, 2026-09-23 (local stack only).** Scope: `engine_snapshot` + `loadEngineInput`.
    ...
    - **AD-93 amendment (2026-09-23):** a snapshot failing Zod is a `RepositoryError`. Logged in the tracker as an amendment to D28.
...
**UI sprint first:** iterate U9 v1 on `u9-first-screen` (tracker lists what is not done). **When U6 resumes**, first take `apps/web/package.json`, `tsconfig.json` and `vitest.config.ts` (and the lockfile) from `u9-first-screen` unchanged, so the branches merge clean. **Check in with the user before implementing** (their instruction, 2026-09-23). Then take chunk 7's red phase to green: migration 7 `public.engine_snapshot` and the Zod-backed `loadEngineInput`, on the local stack only. T-RP1 green again is the proof: any red there is a canary and gets its stop-and-diagnose treatment. Dev stays untouched until the user has verified read-only mode on both sides, and TD-7 wants their decision before the next dev apply.
